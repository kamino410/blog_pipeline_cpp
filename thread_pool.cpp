...
int main() {
  ...
  for(size_t i = 0; i < 3; i++) {
    buffers_1to2_free.push(data_1to2_buf + i);
  }
  ...
  std::thread thread1([&log_thread1, &flag_exit, &buffers_1to2_used, &buffers_1to2_free, &mutex_buffers_1to2_used, &mutex_buffers_1to2_free, &condition_1to2] {
    int id = 1;
  ...
  for(size_t th_id = 0; th_id < 2; th_id++) {
    thread2s.push_back(std::thread([th_id, &log_thread2s, &flag_exit, &buffers_1to2_used, &buffers_1to2_free, &mutex_buffers_1to2_used, &mutex_buffers_1to2_free, &condition_1to2] {
      size_t log_id = 1;
      while(true) {
        int *data;
        {
  ...
  for (size_t th_id = 0; th_id < 2; th_id++) {
    ...
    for (size_t i = 1; i < 20; i++) {
      ...
      std::cout << duration_cast<microseconds>(log_thread2s[th_id][i].checkpoint1 - tm_start).count() << ", ";
      std::cout << duration_cast<microseconds>(log_thread2s[th_id][i].checkpoint2 - tm_start).count() << ", ";
      std::cout << duration_cast<microseconds>(log_thread2s[th_id][i].checkpoint3 - tm_start).count()
                << std::endl;
    }
    std::cout << "</th2-" << th_id+1 << ">" << std::endl;
  }
